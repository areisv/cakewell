...
/* SafeHtmlChecker - checks HTML against a subset of 
   ...
   Tom at klenwell@gmail.com, 24 Jan 2009
   ...
   Update Jan 2009:
        - added additional tags: div, span, b, i, tt
        - restructured create so that parser can be used multiple times in one
          instance of SafeHtmlChecker
        - modified create to return more detailed error info

*/

// Entity classes, adapted from XHTML 1.0 strict DTD
define('E_BLOCK_CONTENTS', 'dl ul ol blockquote p div');
define('E_INLINE_CONTENTS', 'em strong dfn code q samp kbd var cite abbr acronym sub sup a #PCDATA span b i tt');
...
class SafeHtmlChecker {
    ...
    function check($xhtml) {
        ...
        /*
          Create a new parser for each parse.  Otherwise, errors will not be
          cleared.
        */
        $parser = $this->create_parser();
        if ( !xml_parse($parser, $xhtml) ) {
            $this->errors[] = xml_error_string(xml_get_error_code($parser));
        }
    ...
    function tag_open($parser, $tag, $attrs) {
        ...
        if (!in_array($tag, array_keys($this->tags))) {
            $this->errors[] = "Tag <code>$tag</code> not allowed";
            $this->stack[] = $tag;
            return;
        }
